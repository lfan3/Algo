/**
 * 
 * k is the key we want to update,
 * get the key's index: ki = map[k],
 * then use ki with the IPQ
 */
export interface PriorityQueue{
  delete(ki:kiType):Item;
  valueOf():void;
  contains(ki:kiType):void;
  peekMinKeyIndex(): void;
  pollMinKeyIndex(): void;
  peekMinValue(): void;
  pollMinValue(): Item;
  insert(ki:kiType, value:number): void;
  update(ki:kiType, value:number): void;
  decreaseKey(ki:kiType, value:number): void;
  increaseKey(ki:kiType, value:number): void;
  getSize():number;
  getHeap(): Item[];
}

type kiType = number | string;
type Item = {
  ki:kiType ;
  value:number;
}
...
// 相当于map
type posKiHashtable = Record<number, kiType>;
type kiPosHashtable = Record<kiType, number>;
...
export default class IPQ implements PriorityQueue{
  minheap: Item[] = []; //heap.length would not change in my implementation
  heapSize = 0;
  posKiMap: posKiHashtable = {};
  kiPosMap: kiPosHashtable = {};
  
  insert(ki:kiType, value:number){
    this.heapSize++;
  ...
  bubbleDown(pos:number){
    ...
    while(pos>=0 && pos<this.heapSize){
      ...
      if(l>= this.heapSize || this.minheap[min].value > this.minheap[pos].value) break;
      this.swap(min, pos);
      // forget
      pos = min;
    }
